...
router.post('/', (req: Request<Record<string, unknown>, Record<string, unknown>, DataErasureRequestParams>, res: Response, next: NextFunction): void => {
  void (async () => {
    ...
    try {
      ...
      // Never forward a client-supplied `layout` into the view engine: the hbs
      // engine treats a `layout` render-local as a filesystem path to read and
      // wrap the view, which would allow arbitrary local file disclosure.
      const { layout, ...bodyWithoutLayout } = req.body
      if (req.body.layout) {
